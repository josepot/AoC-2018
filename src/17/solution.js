...
const fillUpTank = position => {
  ...
  do {
    ...
    if (!isLeftStream && !isRightStream) {
      const start = getPositionFromIdx(endLeft);
      const end = getPositionFromIdx(endRight);
      for (let i = start.x + 1; i < end.x; i++)
        tankWater.add(getIdx({x: i, y: start.y}));
    }
